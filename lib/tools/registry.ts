...
const TOOL_META: Record<string, Partial<AgentTool>> = {
  ...
  "vision.read_context": { capabilities: ["vision", "file_read"], timeoutMs: 10_000, sideEffects: [], resultSchema: "{ok, output: {context}}", runtimeAvailability: ["claude-code"] },
  "code.python.exec": { capabilities: ["code_execution"], timeoutMs: 35_000, sideEffects: ["code-execution", "process-spawn"], resultSchema: "{ok, output: string<10KB>}", runtimeAvailability: ["claude-code"] },
