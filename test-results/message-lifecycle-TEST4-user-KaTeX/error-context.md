...
- Name: message-lifecycle.spec.ts >> TEST4 user KaTeX
- Location: tests\e2e\message-lifecycle.spec.ts:49:5
...
Locator: locator('.msg-parts.user .msg-text .katex').first()
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for locator('.msg-parts.user .msg-text .katex').first()
...
- main:
  ...
  - button "＋"
  - text: AI
  - heading "只把最强模型放在入口。" [level=2]
  - paragraph: OpenCode Go 始终可独立使用；配置 Anthropic Key 后会自动加入 Claude。联网、URL 和文件内容都只经服务端授权接口处理。
  - text: E2E-MOCK
  - button "◎ 自动联网"
  - button "⚙ 参数"
  - text: 未选择模型 ＋
  - textbox "问点什么，或粘贴 URL…": 一个质量为 \(m\) 的小珠沿半径为 \(R\) 的圆环运动，角速度为 \(\omega\)。
  - button "↑"
  - text: 历史正文保存在本机 · 附件内容不落盘，刷新后需重新添加 · API Key 只在服务端 · URL/联网使用 Exa MCP
```
