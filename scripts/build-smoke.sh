...
check_standalone() {
  local output
  output="$(docker run --rm "$IMAGE" sh -c 'test -f server.js && test -f scripts/task-worker.cjs && test -f scripts/db-migrate.cjs && test -f scripts/schema.sql && echo STANDALONE-OK || echo STANDALONE-MISSING')"
...
for i in 1 2 3; do
  echo "-- build #$i --"
  docker build -t "$IMAGE" "$PROJECT_DIR" > /tmp/build-smoke-$i.log 2>&1
  check_standalone
done
